/**
 * Persisted Sanctuary 3D preferences: the GL crash latch and the kid-facing
 * "Simple view" toggle. Stored as a tiny JSON file in the app document
 * directory -- local-only, no PII, no analytics.
 *
 * The store hydrates asynchronously on first import; until then the defaults
 * (no crashes, 3D allowed) apply, which is safe because a crash during that
 * window still gets recorded and write-through persisted.
 */
...
function prefsFile(): File {
  return new File(Paths.document, "sanctuary3d-prefs.json");
}
...
export const useSanctuary3DPrefs = create<PrefsState>((set, get) => ({
  ...DEFAULTS,
  hydrated: false,
  recordGlCrash: () => {
...
void loadPersisted().then((persisted) => {
  useSanctuary3DPrefs.setState({ ...persisted, hydrated: true });
});
