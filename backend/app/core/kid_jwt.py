"""Hinterland RS256 kid JWT mint + verify + JWKS publication.
...
* ``session`` -- 30-day token the kid app sends as the bearer credential on
  every subsequent request. Verified by :func:`verify_kid_jwt`.

Both tokens use the same kid header (``settings.kid_jwt_kid``) and the same
RSA key pair, which lives in Azure Key Vault and is loaded by
:mod:`app.core.key_vault`.
...
class InvalidKidJwt(Exception):
    """Raised when a Hinterland RS256 JWT fails signature, claim, or type checks."""


class KidTokenClaims(TypedDict, total=False):
...
def _build_payload(
    *,
    ...
    payload: dict[str, Any] = {
        "iss": settings.kid_jwt_issuer,
        "aud": settings.kid_jwt_audience,
        "sub": kid_user_id,
...
def mint_handoff_token(
    *,
    ...
    payload = _build_payload(
        ...
        settings=settings,
        token_type="handoff",
        ttl_seconds=settings.kid_handoff_ttl_seconds,
        jti=jti,
    )
...
def mint_session_token(
    *,
    ...
    payload = _build_payload(
        ...
        settings=settings,
        token_type="session",
        ttl_seconds=settings.kid_session_ttl_seconds,
        jti=jti,
    )
    token = _encode(payload, settings=settings)
    log.info(
...
def verify_kid_jwt(
    ...
    *,
    ...
    issuer, missing required claims) as :class:`InvalidKidJwt` so
    ...
    try:
        decoded = jwt.decode(
            ...
            public_pem,
            algorithms=["RS256"],
            audience=settings.kid_jwt_audience,
            issuer=settings.kid_jwt_issuer,
            options={
        ...
        raise InvalidKidJwt(str(exc)) from exc
    ...
    return cast(KidTokenClaims, decoded)
...
def _public_jwks_for_kid(kid: str, public_pem: bytes) -> dict[str, object]:
    ...
    if not isinstance(key, RSAPublicKey):
        raise InvalidKidJwt("kid-jwt public key is not RSA")
...
def public_jwks(settings: Settings) -> dict[str, object]:
    ...
    """
    public_pem = get_kid_public_pem(settings)
    return _public_jwks_for_kid(settings.kid_jwt_kid, public_pem)
...
# Backward-compatible import aliases for the Dragonfly-era API names.
InvalidDragonflyJwt = InvalidKidJwt
DragonflyTokenClaims = KidTokenClaims
verify_dragonfly_jwt = verify_kid_jwt
