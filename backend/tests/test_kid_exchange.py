...
The tests stub ``verify_kid_jwt`` and ``mint_session_token`` so the
...
def _route_ready() -> bool:
    return hasattr(auth_routes_module, "verify_kid_jwt") and hasattr(
...
def test_kid_exchange_expired_handoff_rejected(
    ...
    """An expired handoff JWT bubbles InvalidKidJwt -> 401."""
    if not _route_ready():
        pytest.skip("kid-exchange route not present yet")
    if not hasattr(auth_routes_module, "InvalidKidJwt"):
        pytest.skip("InvalidKidJwt not present yet")

    invalid = auth_routes_module.InvalidKidJwt("Signature has expired")
...
def test_kid_exchange_wrong_audience_rejected(
    ...
    """A token failing audience check raises InvalidKidJwt -> 401."""
    if not _route_ready():
        pytest.skip("kid-exchange route not present yet")
    if not hasattr(auth_routes_module, "InvalidKidJwt"):
        pytest.skip("InvalidKidJwt not present yet")

    invalid = auth_routes_module.InvalidKidJwt("Invalid audience")
