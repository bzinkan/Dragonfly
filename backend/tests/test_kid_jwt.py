...
* ``verify_kid_jwt(token, *, settings, expected_token_type=None) -> dict``
* ``public_jwks(settings) -> dict``
* ``InvalidKidJwt`` exception
...
def test_mint_handoff_token_contains_expected_claims(
    ...
    assert decoded["iss"] == settings.kid_jwt_issuer
    assert decoded["aud"] == settings.kid_jwt_audience
    # 15-minute handoff TTL by default.
    assert decoded["exp"] - decoded["iat"] == settings.kid_handoff_ttl_seconds

    header = pyjwt.get_unverified_header(token)
    assert header["alg"] == "RS256"
    assert header["kid"] == settings.kid_jwt_kid
...
def test_mint_session_token_uses_session_ttl(
    ...
    assert decoded["exp"] - decoded["iat"] == settings.kid_session_ttl_seconds
...
def test_verify_rejects_expired_token(
    ...
    """An expired handoff JWT must raise InvalidKidJwt."""
    kid_jwt = _kid_jwt_or_skip()

    # Force the TTL to a negative value so the minted token is born expired.
    monkeypatch.setattr(settings, "kid_handoff_ttl_seconds", -10)
    ...
    with pytest.raises(kid_jwt.InvalidKidJwt):
        kid_jwt.verify_kid_jwt(token, settings=settings)
...
def test_verify_rejects_wrong_audience(
    ...
    """A token whose aud claim doesn't match settings.kid_jwt_audience -> reject."""
    import jwt as pyjwt
    ...
    token = pyjwt.encode(
        ...
        private_pem,
        algorithm="RS256",
        headers={"kid": settings.kid_jwt_kid, "alg": "RS256", "typ": "JWT"},
    )
...
def test_verify_rejects_bad_signature(
    ...
    with pytest.raises(kid_jwt.InvalidKidJwt):
        kid_jwt.verify_kid_jwt(tampered, settings=settings)
...
def test_public_jwks_returns_rsa_key(
    ...
    assert key["use"] == "sig"
    assert key["kid"] == settings.kid_jwt_kid
